...
- **Unified Intelligence Hub**: A central data pipeline aggregates data from various sources (ESPN, The Odds API, Open-Meteo) on a 60-second cycle, providing a unified `IntelligenceFeed` to all consuming engines and the frontend.
...
- **Advanced Analytics & Predictive Engines**: Incorporates Continuous Learning, Quantum Fusion (integrating 46 factors), Scheme Recognition, Monte Carlo simulations with advanced Kelly Criterion, and a Strategy Advisor.
...
- **Player Props Analyzer**: A dedicated `/player-props` page shows real-time over/under prop lines, with recommendations and confidence levels. Includes a "Top Props Engine" for surfacing best player prop picks.
...
- **Subscription Gate**: Free-tier users (subscriptionTier='free') are blocked from the app and shown the Pricing page instead. Gate is in `AppContent()` in `App.tsx`. Exempt paths: /pricing, /settings, /legal, /help, /changelog, /track-record. Admin bypasses gate. `/api/auth/check` now returns `tier` field. Backend `requireSubscription` middleware (in `helpers.ts`) blocks free users from intelligence/predictions API routes (402 response). Internal tier names: `pro`=Sharp ($49), `elite`=Edge ($99), `whale`=Max ($249). Admin gets `whale` tier automatically.
- **Email Verification System**: After registration, a 6-digit code is sent via Resend (`server/emailService.ts`). Code stored in-memory (`server/emailVerification.ts`, 15-min TTL). `POST /api/auth/verify-email` validates code and marks user verified. `POST /api/auth/resend-verification` resends with 60s rate limit. `GET /api/auth/verification-status` returns masked email. `/api/auth/check` returns `emailVerified` field. New route `/verify-email` gates app access until verified. Admin and existing users bypass. Requires `RESEND_API_KEY` env var (graceful degradation without it). `emailVerified` boolean column added to users table.
- **Settings Mobile Dropdown**: On mobile screens (below `md:` breakpoint), the 7-tab settings page shows a shadcn `<Select>` dropdown instead of the overflowing horizontal tab list. Desktop keeps the tab bar unchanged. Controlled by `activeTab` state in `Settings` component. Select has `data-testid="select-settings-tabs"`.
- **Historical Backtest Engine**: `server/backtestEngine.ts` fetches 45 days of completed ESPN games and generates retroactive picks (spread and total) against actual final scores. Runs on startup if settled picks < 150. Admin endpoint: `POST /api/admin/backtest/run`. Picks flagged with `isBacktest: true` in TrackedPick. Track Record page shows methodology banner and Live/Backtest filter toggle. `/api/track-record` returns `backtestPickCount` and `livePickCount`.
- **CLV-Gated Learning Engine**: Learning engine categorizes settled picks into 4 buckets by CLV result (strong signal, pure signal, noise, true miss) and applies different weight multipliers to each (1.0, 0.4, -0.2, -0.8). Separate factor weights per market type (spread, total, moneyline) with fallback to unified weights under 30 samples. Weekly calibration check in `calibrationEngine.ts` detects confidence tier drift and applies correction factors.
- **Smart Leg Selector UX**: `MatchupTicketCard` in command center now has individual leg checkboxes, defaulting to the top 3 legs by EV×grade score. Quick action bar: "Best 3", "All", "None" buttons. Live combined odds display updates as legs are toggled. Correlated leg warning appears when multiple positive-correlation legs are selected. "Add N Selected Legs" button replaces "Add All".
- **Model Health Chip**: `ModelHealthChip` component on Command Center shows status (calibrated/building/recalibrating) with colored dot. Clicking opens a popover with settled count, win rate, live vs backtest split, last updated, and "57-factor model" note. Fetches from `GET /api/model-health` (no auth required, 5-min refresh). Status: building if <100 settled, calibrated if win rate 48-60%, recalibrating otherwise.
- **Transparent Branding**: User-facing display text updated: "Quantum Fusion Analysis" → "46-Factor Model Analysis", "Quantum Fusion" engine badges → "Multi-Factor Engine", "Quantum Coherence" → "Model Agreement". Code variable names unchanged. Only JSX string literals changed.
- **Onboarding Guard Admin Bypass**: `OnboardingGuard` component now accepts `isAdmin` prop. Admin sessions skip the onboarding API query entirely (`enabled: !isAdmin`) and bypass the redirect. `user_onboarding` table created in PostgreSQL. Exempt paths extended to include `/verify-email`, `/changelog`, `/track-record`.
- **SSE picks-settled Integration**: `use-sse.ts` now handles `picks-settled` event type. `BetTracker` subscribes via `useSSE({ onEvent })` and auto-invalidates `/api/user/bets`, `/api/user/bet-stats`, and `/api/user/picks/clv-summary` queries when picks settle. Shows a win/loss toast notification with game and score.
- **CLV Performance Card in BetTracker**: Overview tab now shows a "Closing Line Value (CLV) Performance" card when settled picks exist. Displays CLV+ Rate, Avg CLV (basis points), and current CLV streak. Fetches from `/api/user/picks/clv-summary`. Only visible when `settledPicks > 0`.
- **Backtest Engine Fix**: `initBacktestOnStartup()` now triggers on `getBacktestCount() === 0` (no backtest data) instead of `total < 150`. This ensures historical backtest always runs on fresh deployments even with many live picks. Backtest generates 2,000+ historical picks from 45 days of ESPN data across NBA, NHL, MLB, NCAAB.
...
- **Email**: Resend (transactional email for verification, welcome, and weekly digest — requires RESEND_API_KEY)