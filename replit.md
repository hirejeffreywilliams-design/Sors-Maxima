...
- **Player Prop Track Record**: `prop_track_records` PostgreSQL table (created via raw SQL). Every high-confidence (≥55) prop recommendation is auto-saved when `POST /api/prop-parlays` is called. Admin can settle results via `PATCH /api/prop-track-record/:id/settle`. Stats exposed at `GET /api/prop-track-record/stats`. Frontend collapsible section at bottom of `/player-props` page shows overall record, per-market win rates, and recent picks.
- **Enhanced Prop Intelligence Engine**: `server/propParlayEngine.ts` upgraded with: (1) Public Over-bias correction — detects when books shade lines up on Over (structural Under value), (2) Sharp signal detection — analyzes bookmaker odds dispersion and reverse book pricing for sharp movement, (3) Historical learning feedback — queries `prop_track_records` by market type and adjusts model confidence based on past accuracy (15-min cache), (4) Market accuracy cache `getMarketAccuracyStats()` exposed as export for system learning.
- **DB Safety Note**: `prop_track_records` was created with raw SQL (NOT `npm run db:push`) because db:push detected rename conflicts with existing tables. All prop track record operations use raw `db.execute(sql.raw(...))` to bypass drizzle ORM.
