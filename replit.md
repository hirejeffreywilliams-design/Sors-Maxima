...
- **Orchestration System**: Advanced system-level orchestration (server/ticketingEngine.ts, server/confidenceEngine.ts, server/featureRegistryEngine.ts, /admin/orchestration) implementing comprehensive sports betting platform governance. Includes: Ticketing Engine with 8 ticket types (incident, human_review, business_rule_violation, payment_issue, fraud_alert, kyc_issue, system_alert, confidence_ticket), priority/SLA mapping (P0-P4), lifecycle management, retry/dedupe with idempotency keys, and observability metrics. Confidence Recommendation Engine with probabilistic bet recommendations, Kelly criterion stake sizing, model provenance tracking (4 models: ensemble, gradient_boost, neural_network, hybrid), calibration bin validation, auto-send policy with configurable thresholds, bankroll/exposure rules, and human-in-the-loop for low-confidence/high-stakes picks. Feature Responsibility Registry with 8 registered features (KYC, Risk Management, Responsible Gambling, Fraud/AML, Pricing, Payments, Recommendation Engine, Notifications), health scoring, compliance tracking, coordination rules (8 rules including safety priority override, transactional consistency, calibration drift response), business/ethical constraints (10 constraints covering dark patterns, PII minimization, jurisdiction compliance), and inter-feature event bus. Admin dashboard with 8 tabs: Overview, Tickets, Confidence Tickets, Recommendations, Model Performance, Feature Registry, Coordination, and Policies.
