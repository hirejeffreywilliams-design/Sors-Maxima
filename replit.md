...
- **Crash Guard & Memory Safeguards**: Process-level `uncaughtException` and `unhandledRejection` handlers prevent total app crashes. Engine startups wrapped in try-catch. Memory limits reduced across all engines: Platform Intelligence (2000 games, 1500 predictions, 1000 odds), Monte Carlo (100 cache entries), Analytics Agent (200 history entries, 300 odds keys), Prop Snapshots (500 max). Guardian triggers `global.gc()` on >90% heap. Null-safe guards on frontend data access patterns.
