...
- **Admin Operational Tools (Main Dashboard `/admin`)**: Four new tools added directly to the command center: (1) **Platform Broadcast** — "Broadcast" tab with compose/preview/send UI for platform-wide or tier-targeted announcements that inject into the live notification feed; (2) **Emergency Controls** — orange card in Overview tab with one-click buttons for Force Refresh Picks, Push SSE to All Clients, Clear Intelligence Cache; (3) **User Detail Panel** — "View" button on each user row opens a profile dialog showing subscription, onboarding data, and recent picks; (4) **Force Tier Change** — "Tier" button on each user row opens a direct DB override to change any user's subscription tier without Stripe. Backend routes: `POST /api/admin/broadcast`, `GET /api/admin/users/:id/profile`, `POST /api/admin/users/:id/change-tier`, `POST /api/admin/emergency/force-refresh-picks`, `POST /api/admin/emergency/force-sse-push`, `POST /api/admin/emergency/clear-intelligence-cache`.
