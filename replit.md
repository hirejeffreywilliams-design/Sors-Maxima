...
- **Monte Carlo Vertical Agent & Pre-Game Overdrive Engine (Task #26)**: Elevates simulation intelligence into a dedicated Simulation Specialist vertical AI agent (`server/monteCarloVerticalAgent.ts`). Routes simulation queries automatically from `/api/ai/analyst` to this specialist. Adds 5 new factors to a 51-factor model (referee crew bias, player micro-matchups, coach tactical tendencies, sentiment insider signal, travel quality scoring). Implements a 3-wave pre-game overdrive system (`server/overdriveEngine.ts`): Wave 1=250K sims at 24h, Wave 2=500K at 12h, Wave 3=1M+ at 2-3h before game. `server/overdriveInputBuilder.ts` enriches simulation inputs with all 5 new factors. A `SimulationDepthIndicator` component shows sim count and tier (Good/Strong/Elite/Overdrive Elite) on every pick card. New API endpoints: GET /api/simulation/overdrive-status, GET /api/simulation/depth/:gameId, GET /api/simulation/factors. All 5 factors added to quantumFusionEngine.ts FUSION_WEIGHTS and FACTOR_CATEGORIES.
