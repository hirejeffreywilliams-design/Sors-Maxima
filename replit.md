...
- **Sors Intelligence Cards**: A collectible trading card system with visual effects (rainbow holographic shimmer, sport-specific backgrounds, rarity labels, 3D tilt hover, grade-matched glow borders, "CALLED IT/NO HIT" branded result stamps, circular grade ring badges, large "S" monogram brand watermark, 🔥 HIGH EDGE badge for EV≥15, stats grid on card back, "↺ flip" engagement hint). Null guards on grade/betType/sport prevent crashes from incomplete card data.
...
- **Admin Profile Fix**: Admin login now calls `getOrCreateAdminUser()` in `dbAuthService.ts` to ensure the admin always has a real DB row, storing their numeric user ID in the session. Fixes 500 errors on `/api/user/betting-profile`, `/api/cards/collection`, and related profile endpoints that previously received NaN from `parseInt("admin")`. Safe fallback: betting-profile GET returns default empty data for null userId instead of crashing. `sessionUserId()` helper added to `server/routes/cards.ts` for all 7 card endpoints.
