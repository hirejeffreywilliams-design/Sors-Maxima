...
- **Consolidated Navigation (6 Primary Items)**: Desktop nav: Picks | Daily | Build ▾ (Smart Generator, Visual Builder, Strategy Advisor) | Markets ▾ (Odds Center, Player Props, Global, Live Scores) | Tools | Community. Mobile: Sheet drawer with sectioned MobileNav. BottomNav: Picks/Daily/Build/Markets/More. Desktop header has UserMenu avatar dropdown replacing flat username+logout.
- **Advanced Analytics & Predictive Engines**: Incorporates Continuous Learning, Multi-Factor Intelligence Engine (46 data-backed factors), Scheme Recognition, Monte Carlo simulations with advanced Kelly Criterion, and a Strategy Advisor.
...
- **Life Changer Ticket**: A daily cross-sport underdog parlay generator. `buildLifeChangerTicket()` in `precomputedPredictionsEngine.ts` selects legs from 4 pools: underdogs (+110 to +450), contrarian picks (going against the public with positive EV), alternative markets (totals, 1H, team totals), and sleepers (+200 to +600). Soccer picks from the International Engine are automatically injected into the pool — draws (+200 to +350) and underdog moneylines add substantial combined odds. Uses a date-seeded shuffle for consistent daily picks. Served at `GET /api/life-changer-ticket`. The Command Center shows an amber-styled card with combined odds, payout grid ($10/$25/$50/$100), expandable leg list with category badges, and one-tap "Add All to Slip".
- **International Sports Engine**: `server/internationalSportsEngine.ts` fetches fixtures from API-Football for 8 soccer leagues (EPL, La Liga, Bundesliga, Serie A, Ligue 1, Champions League, MLS, International) with ESPN fallback. Fetches real odds from The Odds API using soccer sport keys (h2h/totals, regions us/uk). Generates 4 pick types per fixture: away underdog ML, draw (+180 to +400), home value ML, and over goals. Starts 145s after server boot, refreshes every 15 minutes. Routes: `GET /api/international/feed`, `GET /api/international/picks`, `GET /api/international/league/:sport`. Frontend at `/international` ("Global" nav item) with league filter grid, sort by EV/odds/confidence, and "Add to Slip" integration.
