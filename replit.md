...
- **Autonomous Learning Engine**: `server/autonomousLearningEngine.ts` — bootstraps the Sors Simulation Engine (MC engine) and Stacking Meta-Learner from all 477 historical settled picks on startup. Every future settled pick auto-feeds Bayesian priors and sport calibration. Hourly learning cycles keep models current. Bootstrap state persisted to `autonomous-learning-log.json`. Admin status visible at Admin → Model Performance → Auto-Learn tab (`/api/admin/autonomous-learning`). Manual trigger: `POST /api/admin/actions/run-autonomous-learning`.
...
- **Grade Ambient Glow System**: All pick cards and ticket cards throughout the app display a grade-matched ambient box-shadow glow. Utility at `client/src/lib/grade-utils.ts` — `gradeAmbientGlow(grade)` returns inline style, `getGradeShimmerClass(grade)` returns animated shimmer border class for A/A+ cards. Applied to command-center, precomputed-picks, ticket-showcase, and pick-highlight-cards. CSS keyframes in `client/src/index.css`.
- **Sors Intelligence Cards**: Collectible trading card system at `/cards`. DB tables: `trading_cards`, `user_card_collections`, `card_trades` (defined in `server/dbSchema.ts`, raw SQL in `server/dbMigrations.ts`). API routes in `server/routes/cards.ts` under `/api/cards/*`. Card visual component in `client/src/components/trading-card.tsx` — grade-based holographic shimmer, 3D tilt hover, CALLED IT/MISSED stamps, pending pulse. Cards page (`client/src/pages/cards.tsx`) has My Collection, Marketplace, Trades tabs. Pack drops tier-gated: Sharp=3/week, Edge=5/week, Max=daily. High-grade picks (A-/A/A+) auto-minted with limited copies (50/25/10) by precomputed predictions engine. Card Showcase section on profile page. "Cards" added to bottom nav pool.
