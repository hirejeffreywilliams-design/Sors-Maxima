...
**Season-Aware Sport Filtering**: Centralized season detection utility (`server/sportSeasons.ts`) defines real-world season windows for all 6 sports. All engines (Intelligence Hub, Precomputed Predictions, Platform Intelligence, Monte Carlo, Roster Preloader, Ticket Orchestrator, Community alerts) call `getInSeasonSports()` instead of hardcoding all 6 sports. Off-season sports are automatically skipped to save API calls, memory, and processing time. Season windows: NBA (Oct-Jun), NFL (Aug-Feb), MLB (Mar-Nov), NHL (Oct-Jun), NCAAB (Nov-Apr), NCAAF (Aug-Jan).
...
- **Advanced Monte Carlo Engine**: Dedicated simulation engine (`server/monteCarloEngine.ts`) with MersenneTwister RNG, Sobol quasi-random sequences, Latin Hypercube sampling, antithetic variates, importance sampling for rare events, and adaptive simulation count. Models game outcomes using Poisson (NHL/MLB) and Normal (NBA/NFL/NCAAB/NCAAF) score distributions. Runs 10k matchup simulations per game and 50k parlay simulations (optimized from 25k/100k for memory efficiency). Background pre-simulation cycle every 2 minutes across up to 30 upcoming games with TTL-based cache eviction. Self-learning via Bayesian updating with drift detection, calibration history, and accuracy tracking. Risk analytics: VaR, CVaR, Kelly, ruin probability. Persists to `monte-carlo-learning-data.json`. Integrated with Quantum Fusion Engine (real simulation data for monte_carlo signal), Precomputed Predictions Engine (Monte Carlo score predictions and spread/total estimates), storage.ts evaluateParlay/generateOptimalParlays, Platform Intelligence Engine (outcome recording for self-learning), and Continuous Learning Orchestrator (calibration drift checks). API endpoints: `GET /api/monte-carlo/status`, `GET /api/monte-carlo/calibration`, `GET /api/monte-carlo/pre-simulated`, `GET /api/monte-carlo/simulate/:gameId`, `POST /api/monte-carlo/simulate`. Frontend dashboard in Platform Intelligence page under "Monte Carlo" tab showing engine status, calibration buckets, pre-simulated games, and self-improvement trends.
- **Precomputed Predictions Engine**: Runs every 5 minutes across in-season sports only (determined by `sportSeasons.ts`), combining Quantum Fusion analysis with Vegas engine predictions. Results displayed on Daily Picks page and fed into Command Center top picks.
