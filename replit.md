...
- **Command Center (Ticket-First)**: The primary dashboard (`/`) leads with "Today's Best Tickets" — ready-to-place parlays auto-assembled by `buildOptimalTickets()` in `server/precomputedPredictionsEngine.ts` from the highest-graded precomputed picks. Each ticket shows combined grade, total odds, Kelly-optimized stake, payout, engine convergence indicators, and an "Add All to Slip" one-click button. Below tickets: stat cards (Best Grade, Highest EV, Live Games), then Top Picks, Edge Alerts, Live Games, and Engine Convergence sidebar. Served by `GET /api/optimal-tickets?sports=&risk=&bankroll=&maxLegs=` endpoint.
...
- **Persistent Floating Bet Slip**: A persistent slide-out bet slip (`client/src/components/parlay-slip-drawer.tsx`) accessible from every page via a floating button. Features: stake input with preset quick buttons ($10/$25/$50/$100), dynamic payout calculation, branded "Place at" sportsbook buttons (DraftKings, FanDuel, BetMGM, Caesars, PointsBet, BetRivers) that copy the formatted slip and open the book, one-tap copy/share of the full slip, and compact affiliate disclosure. The Export Bet Slip page (`client/src/components/export-bet-slip.tsx`) auto-pulls legs from the ParlaySlip context when available.
