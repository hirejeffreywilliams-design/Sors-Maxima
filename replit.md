...
- **Reactive Reasoning System (Phases 1–3)**: Signal Translation Layer (`server/signalTranslationLayer.ts`) replaces inline text generation with a threshold-driven reasoning engine. Pick Snapshot Store (`server/pickSnapshotStore.ts`) tracks first-seen values per pick and computes `confidenceDelta`, `edgeDelta`, `sharpMoneyDelta` between model runs. Upgrade Badges (`wasUpgraded`, `wasRevised`, `signalStrengthened`) detected per-pick and surfaced in the API. Frontend shows delta arrows (↑/↓) on confidence/edge boxes and Upgraded/Revised/Signal↑ badges in pick cards on Dashboard, Command Center, and Daily Parlays.
- **Founders Program Backend (Task #12)**: Complete "Founding 500 + First 5 Enterprise" launch engine. 9 founder columns added to `users` table via migrations (`is_founder`, `founder_number`, `founder_type`, `founder_joined_at`, `founder_price_locked_tier`, `founder_price_locked_amount`, `founder_referral_code`, `founder_referral_count`, `founder_credits_earned`). New `founders_program` table for global program state. `server/foundersEngine.ts` handles: `getProgramStatus()`, `launchProgram()`, `autoGrantOnSubscription()`, `manualGrant()`, `revokeFounder()`, `getFoundersForWall()`, `getTierBoost()`, `checkEarlyAccessWindow()`. Admin routes at `POST /api/admin/founders/launch`, `GET /api/admin/founders/status`, `POST /api/admin/founders/grant`, `DELETE /api/admin/founders/revoke/:userId`. Public route at `GET /api/founders/status`. Auth check response enriched with founder fields. Stripe webhook auto-grants founder status on active subscription. Founder welcome emails (member + enterprise) and announcement email added to `server/emailService.ts`.
