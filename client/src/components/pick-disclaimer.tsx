...
export function PickDisclaimer({ variant = "inline" }: PickDisclaimerProps) {
  ...
  if (variant === "compact") {
    return (
      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <span>46-Factor analysis — not guaranteed outcomes. </span>
        <Link href="/track-record" className="text-primary hover:underline flex items-center gap-0.5">
          See track record <ExternalLink className="h-2.5 w-2.5" />
        </Link>
      </div>
    );
  }
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Picks are generated by our 46-Factor Intelligence Engine — not guaranteed outcomes. Never bet more than you can afford to lose.{" "}
  ...
  return (
    <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/30 border border-border/40 rounded-md p-2.5">
      ...
      <div className="flex-1 min-w-0">
        <span>
          46-Factor Intelligence Engine analysis — not financial advice. Never risk money you cannot afford to lose.
        </span>{" "}
...
export function CalibrationBadge({ confidence }: { confidence: number }) {
  ...
  if (!data || !data.hasMinimumData) {
    return (
      <span className="text-xs text-muted-foreground" title="Intelligence projection — not yet verified by real outcomes">
